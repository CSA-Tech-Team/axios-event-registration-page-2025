 ...
 * a perfectly valid session. The stored token is still honoured, for the
 * cookie-less fallback and for sessions predating the migration.
 *
 * Returns `null` while the session is still resolving, so callers can wait
 * rather than act on a not-yet-known answer.
 */
