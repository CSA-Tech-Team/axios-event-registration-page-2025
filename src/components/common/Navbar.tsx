  ...
  return (
    <div ref={navRef} className="relative flex justify-center  h-fit w-screen p-0 ">
      <div className="relative flex justify-center h-fit w-screen p-0">
        <div
          className="bg-indigo-950 mx-5 rounded-xl flex justify-between items-center px-6 py-2 
        ...
        >
          ...
          <div className="flex gap-4">
            ...
            <div
              className={`flex items-center gap-2 px-4 py-2 cursor-pointer transition-all
                ${activeIndex === 0 
                  ? "bg-indigo-200 text-indigo-900 rounded-xl shadow-md" 
                  : "text-white hover:bg-indigo-800 rounded-xl"}`}
            ...
            >
              ...
              <span className="hidden sm:block text-sm font-medium">Events</span>
            </div>
          </div>
          ...
          <div className="flex gap-4">
            ...
            >
              <img src={LeaderBoardIcon} alt="Leaderboard" className="w-5 h-5" />
            ...
            >
              ...
              <span className="hidden sm:block text-sm font-medium" onClick={handleLogout}>
                {isAuthenticated() ? "Logout" : "Signin"}
              </span>
            </div>
          </div>

          <div className="absolute lg:left-[47%] md:left-[43%] sm:left-[50%] transform -translate-x-1/2 -top-5 
                        bg-indigo-700 rounded-full p-2 sm:p-2.5 text-white z-10 cursor-pointer shadow-xl">
            <div
              className="rounded-full bg-indigo-500 p-3 sm:p-4 hover:bg-indigo-400 transition-all"
              onClick={() => {
                handleClick(2);
                setProfileIndex(0);
        ...
        {showNavbar2 && (
          <div>
            <motion.div
              ...
              className="absolute text-white flex items-center justify-center left-[calc(50%-35px)]  lg:left-[calc(45%-15px)] transform -translate-x-1/2 -top-14 w-12 p-4 bg-[#171717] outline outline-white outline-1 rounded-full"
            >
              {profileIndex == 1 ? (
            ...
            <motion.div
              ...
              exit={{ x: -25, y: 100 }}
              transition={{ ease: easeOut, duration: 0.25 }}
              className="absolute flex  left-[calc(50%+45px)]  lg:left-[calc(50%)]  -top-20 w-12 p-4 bg-[#171717] outline outline-white outline-1 rounded-full"
            >
              {profileIndex == 2 ? (
                <img src={profileIcon} alt="" className="w-5 h-5" />
              ) : (
            ...
            <motion.div
              ...
              exit={{ x: -90, y: 50 }}
              transition={{ ease: easeOut, duration: 0.25 }}
              className="absolute flex left-[calc(50%+120px)] lg:left-[calc(55%+15px)] transform -translate-x-1/2 -top-14 w-12 p-4  bg-[#171717] outline outline-white outline-1 rounded-full"
            >
              {profileIndex == 3 ? (
                <img src={profileIcon} alt="" className="w-5 h-5" />
              ) : (
